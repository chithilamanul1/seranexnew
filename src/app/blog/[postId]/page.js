    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-12 gap-8">
      ...
      <aside className="hidden md:block col-span-3">
        <div className="sticky top-8 bg-white p-4 rounded-lg shadow-md border border-gray-200">
          ...
          <ul className="space-y-2 text-gray-700">
            <li><a href="#" className="hover:text-indigo-600 font-semibold">#javascript</a></li>
          ...
          {posts.map(post => (
            <div key={post.id} className="bg-white rounded-lg shadow-md border border-gray-200 flex">
              <div className="flex flex-col items-center p-3 bg-gray-50 rounded-l-lg">
                <button aria-label="Upvote" className="text-gray-500 hover:text-indigo-600"><FaArrowUp size="1.2em" /></button>
                <span className="font-bold text-lg my-1">{post.voteCount || 0}</span>
                <button aria-label="Downvote" className="text-gray-500 hover:text-red-600"><FaArrowDown size="1.2em" /></button>
              </div>
      ...
      <aside className="hidden lg:block col-span-3">
        <div className="sticky top-8 bg-white p-4 rounded-lg shadow-md border border-gray-200">
          ...
          <p className="text-sm text-gray-600">This is a platform for developers to share ideas, ask questions, and connect.</p>
        </div>
      </aside>
    </div>
  );
